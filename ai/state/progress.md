...
[CP-2026-08-22-02] Session: remote pull + reconciliation (dropped stale CP-08-09-02), decision reversal recorded, notes consolidation, [redacted] example, sync-agents-md.sh macOS BSD sed fix (db74e68). Detail in protocol-decisions.md.
...
[CP-2026-09-09-01] Fixed checkpoint procedure never writing the daily checkpoint file: added Procedure C step 2 "Write Daily Checkpoint File" to AGENTS.md (steps renumbered 2->3/3->4/4->5); mirrored in ai-policy-common.md (Daily Checkpoint File Mandate), validate-protocol.sh (v4.7, new anchor check), docs/workflow-guide.md + slides; used this repo's own YYYY-MM-DD.md convention over the issue's proposed per-checkpoint naming; created missing ai/artifacts/ dir; validator 8/8, review-01 APPROVED. Detail in protocol-decisions.md. Committed and pushed as f716105.
[CP-2026-09-09-02] Formalized ai/issues/ in the protocol: added Project Issues Directory (ai/issues/) to AGENTS.md TIER 1, scoped per-project not global; Procedure A Step 2 audits it, Step 5 indexes by filename+line count excluding closed-* files, Step 7 adds Proof-of-Load bullet (g); lifecycle = closed- filename prefix (rejected subdirectories + status field). validate-protocol.sh v4.7->v4.8 (CONFIG_KEYS + PROJECT_SUBS + anchor check). Both ai/issues/ files renamed with closed- prefix, second one's Status updated to Resolved. Validator 8/8, review-02 APPROVED. Detail in protocol-decisions.md. Committed to master as 2149bc8.
