...
I built this for myself, to get my AI assistant to remember my projects, follow my rules and maintain (context) state. It is a handful of plain files. Nothing to install.

## Setup

- **Time**: under 5 minutes.
- **Difficulty**: very low. You should be comfortable with basic computer tasks, like copying files and running a command.
- **What you need**: any AI assistant. It works best with one built into VS Code, like Copilot, Claude, Gemini, DeepSeek, or ChatGPT. You may want to add an official extension for your AI assistant first. If you are new to AI, do not start from an exotic (complicated) setup: have one approved AI extension installed in VS Code before you start.
- **Install**: one `git clone` and a couple of file copies. No install scripts, no admin rights, no package managers.

## Quick start

1. Clone this repo somewhere on your machine (for example `~/Projects/Simple-AI-Workflow`).
2. Create your global AI directory structure - it is important:
  - `mkdir $HOME/.ai`
  - `mkdir $HOME/.ai/settings/`
  - `mkdir $HOME/.ai/global-knowledge/`
  - `mkdir $HOME/.ai/backups/`
  - Optional: copy `docs/global-user-settings.md` to `$HOME/.ai/settings/global-user-settings.md` and fill in your own details.
3. Copy `AGENTS.md` to your project root - by CLI or file manager.
4. Copy `docs/ai-customization.md` to `ai-customization.md` at your project root, then set `Global AI Workflow Directory` inside it to where you cloned this repo.
5. Add expertise, traits, or compliance settings if you want them (see the [customization guide](docs/ai-customization-guide.md)).
6. Open your project in VS Code and type this in the AI chat:
   - `"bootstrap using AGENTS.md protocol"`

> **Note:** don't use `/init`. It behaves differently across AI tools/harnesses. Use the text prompt above instead.

When you finish a session, type `"checkpoint"`. To pick up later, type `"load context using AGENTS.md protocol"`.  You are encouraged / advised to do frequent checkpoints. Think of checkpoint as "ctrl + s" !
...
```text
   ...
   |-- ai/policies/                 |-- ai/ (state, notes, issues)
   |-- docs/                        |-- src/
   |-- support-files/               |-- ...
```

The repo holds the workflow and its policy files. You copy `AGENTS.md` and `ai-customization.md` into your project. The AI creates and maintains the project's `ai/` directory (state, knowledge, issues) as it works.

`AGENTS.md` is kept deliberately short by design (progressive disclosure): everything that applies to every session stays inline, and everything else sits behind a context pointer that the AI opens only when a task needs it.
...
But do you know where its knowledge, state, and rules actually live? What happens when you move the project to another directory, or to another computer? How do you back up the AI state, your project knowledge, and the rules?

The trouble grows when you use two or more assistants on the same project. Without a shared system, each one stores its state in its own hidden directory. Switch assistants and you start from scratch, because the new one has no idea what the last one did.
...
| Review a file, directory, repo, or PR | `"perform a code review on <topic>"` |
| Apply the fixes from a review report | `"apply fixes for problems identified in code review report"` |
...
- *"process my notes directory and remove the notes that are already implemented; for the ones that remain, add a short clarifying comment or a suggestion."*
- *"process my plans directory and find all plans that are fully implemented. If there are any, extract the useful knowledge and save it under `ai/shared/project-knowledge/`."*
- *"find any stale files under plans, artifacts, or notes and tell me about them without deleting anything."*
- *"process the artifacts directory and remove any stale files."*
- *"process my project knowledge and find any stale files, then update or remove them; where it helps, rename them with better, verbose kebab-cased names for easier indexing."*
...
1. **Ideally, it should fire on its own.** When the AI spots a compaction summary in the conversation, it should run the `post-compaction recovery procedure` automatically and reload its loaded rules. Though AI itself misses this most of the time. (*This issue is under active investigation at the moment*.)
...
| Rules lost after auto-summary | Post-Compaction Recovery reloads rules from disk; you can also trigger it manually |
...
- **Checkpoint often.** After each feature, fix, or review cycle, not just at the end of the day. This keeps the active state files small and efficient, and all the information keep getting recorded in the state log / diary which is maintained under daily-checkpoint. This is *extremely* helpful to find out what you did three months ago - for example. The AI will go through all the daily checkpoints, and find out the information you were looking for.
- **Keep sessions shorter.** If the AI starts repeating itself or forgetting earlier constraints, that is your signal. Checkpoint, compact conversation manually using "/compact" or "/compress", run "post-compaction recovery procedure" and continue working.
- **Always type the full `"load context using AGENTS.md protocol"`** at the start of a session. The short `"load context"` is discouraged, as it is unreliable with a fresh or weaker models.

## Customizing the AI

Two files shape how the AI behaves. One is global, one is per project.

**Global: `$HOME/.ai/settings/global-user-settings.md`**

Your own profile: name, role, skills, preferences, writing style, words you don't want the AI to use, and the CLI tools it can call. The AI loads it at the start of every session. Start from `docs/global-user-settings.md`.

**Per project: `ai-customization.md`**

This is the one file you edit to tailor the AI to a project. You can set:
...
4. **Developer Workflow**: set project-specific working rules, for example a proactive peer review after every change set.

See the [AI Customization Guide](docs/ai-customization-guide.md) for the full list and examples.

## Keeping this workflow up to date

> Run the sync script (`support-files/sync-agents-md.sh` or `sync-agents-md.ps1`) and it migrates for you, or follow the [manual steps](docs/ai-customization-guide.md).

When I push updates in this repo, you would want to have those updates on your local computer too, so your projects can benefit from new features and bug-fixes.

So, the process is as follows:

1. Run a checkpoint in your current project first, and ideally close the IDE / AI assistant.
...
**Note:** You *can* simply copy the `AGENTS.md` file from the Simple-AI-Workflow directory (where you pulled latest changes), and paste / overwrite the `AGENTS.md` in the projects you are interested in. However copying in several directories is cumbersome. Also, sometimes the changes are more than just an updated `AGENTS.md` file, so using the sync script is better.

4. Go to the project directory you started from, start the IDEA again (e.g. VsCode), and when it loads, type `"load context using AGENTS.md protocol"` to pick up the new updates.

## Examples

Working examples you can copy:

- [Full-stack web and mobile customization](docs/examples/full-stack-web-mobile-ai-customization.md): a filled-in `ai-customization.md` for a mixed web and mobile project.
- [Handoff examples](docs/examples/handoffs/): what a handoff between sessions looks like.
- [Knowledge base examples](docs/examples/knowledge-base/): how project knowledge is organized.
- [Personas](docs/personas/): example traits for an architect reviewer, a mentor, and a security specialist.
- [Example learning session](docs/example-learning-session-runbook.md): a full walkthrough of a session from setup to checkpoint.
...
- **Policies**: 12 domain policies (Cloud, API Backend, Web Frontend, Data, DBA, Observability, Linux SysAdmin, Windows SysAdmin, Mobile, Accounting, Academic Research, and Career Coaching), plus 4 protocol policies (common, meta, code-review, codebase-examination). Load the ones that apply to your project.
...
Other docs:
- [Workflow guide](docs/workflow-guide.md): handoffs, knowledge base, issue management, state files
- [AI Customization Guide](docs/ai-customization-guide.md)
- [Compliance guide](docs/compliance-guide.md)
- [How policies work in this workflow](docs/how-policies-work-in-this-workflow.md)
...
- [Provider selection guide](docs/ai-provider-selection-guide.md)
- [Beginner setup guide](docs/vscode-cline-provider-setup-for-beginners.md)
- [Agent collaboration notes](docs/ai-agent-collaboration.md)
