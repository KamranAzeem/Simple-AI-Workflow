...
4.  **Context Re-affirmation After Checkpoint (condition-gated)**: A checkpoint normally runs mid-session when policies and knowledge are already in context, so a blanket reload is unnecessary and wasteful. Perform a reload **only** when the active context has been condensed/compacted since the last full load, or when you are otherwise unsure the operational files are still loaded:
    ...
    - Load the FULL TEXT of `ai-policy-common.md`, all files in the **Global AI Knowledge Directory**, every policy file referenced in the **Project Customization File**, and all custom policies discovered in **Project AI Policies Directory** and **Project Compliance Policies Directory**.
...
3. **Load standing rules and knowledge**:
    ...
    - Run a recursive shell command (`find` or equivalent) on **Project AI Policies Directory** to discover all `.md` files. Load the FULL TEXT of every discovered file. These are user-created custom policies — not referenced by the customization file — that must be loaded even on recovery. Compliance policy files under **Project Compliance Policies Directory** are covered automatically by the recursive scan.
